...
def rep_key_xor(s, key):
    s = bytearray(s, "utf-8")
    key = bytes(key, "utf-8")
    xored = bytes([byte ^ key[i % len(key)] for i, byte in enumerate(s)])
    return binascii.hexlify(xored) # to have a "hexadecimal readable string" as output
